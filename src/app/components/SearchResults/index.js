        ...
        {Array.isArray(trips) && trips.map((trip) => (
          <div key={trip.id} className="trip__background rounded-3xl relative">
            <section className="">
              ...
              <button className={`${trip.status === 'COMPLETED' ? 'bg-green' : 'bg-warning'} absolute px-3 py-2 rounded-3xl text-xs text-white left-52 top-3`}>
                {trip.status}
              </button>
            ...
            <section>
              ...
              <div className="absolute top-56 left-4">
                <p className="text-sm">{trip.distance}{trip.distance_unit}</p>
              </div>
            </section>
          </div>
        ))}
